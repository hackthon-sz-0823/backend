...
export class AchievementService {
  ...
  async getUserAchievements(
    ...
    try {
      ...
      const [totalScore, totalClassifications, correctClassifications] = await Promise.all([
      ...
      const userStats: UserStats = {
        ...
        classificationAccuracy: totalClassifications > 0 
        ...
        achievements.map(
          async (achievement): Promise<AchievementWithProgress> => {
            ...
            if (userAchievement) {
              ...
              // 成就完成但没有记录，创建记录
              try {
                const newRecord = await this.prisma.prismaClient.walletAchievement.create({
                  data: {
                    walletAddress: normalizedAddress,
  ...
  async claimAchievement(dto: ClaimAchievementDto): Promise<{
    ...
    try {
      ...
      if (!walletAchievement) {
        ...
        const requirements = achievement.requirements
          ? (JSON.parse(achievement.requirements as string) as AchievementRequirements)
          : undefined;
        ...
        // 创建成就记录
        walletAchievement = await this.prisma.prismaClient.walletAchievement.create({
          data: {
            walletAddress: normalizedAddress,
            achievementId: dto.achievementId,
            progress: 100,
