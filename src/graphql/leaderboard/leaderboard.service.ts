...
export class LeaderboardService {
  ...
  async getLeaderboard(input: GetLeaderboardInput): Promise<LeaderboardResponse> {
    try {
      const { limit = 10, offset = 0 } = input;

      // 使用 groupBy 聚合积分数据
      const scoreGroups = await this.prisma.prismaClient.scoreTransaction.groupBy({
        by: ['walletAddress'],
        where: {
          isValid: true,
        },
      ...
      // 获取总用户数
      const totalUsers = await this.prisma.prismaClient.scoreTransaction.groupBy({
        by: ['walletAddress'],
        where: {
          isValid: true,
        },
  ...
  async getUserRanking(walletAddress: string): Promise<UserRanking> {
    try {
      // 获取用户总积分
      const userScore = await this.prisma.prismaClient.scoreTransaction.aggregate({
        where: {
          walletAddress,
          isValid: true,
        },
      ...
      // 获取所有用户积分并排序
      const allUserScores = await this.prisma.prismaClient.scoreTransaction.groupBy({
        by: ['walletAddress'],
        where: {
          isValid: true,
        },
      ...
      // 找到用户排名
      const rank = allUserScores.findIndex(
        group => group.walletAddress === walletAddress
      ) + 1;
